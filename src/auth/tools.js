...
const generateAccessTokenForGoogle = (payload) => new Promise((resolve, reject) => jsonwebtoken_1.default.sign(payload /* .toJSON() */, // To Json() / Should actually be there. But TS is giving an error , so I have commented it out for now.
process.env.JWT_SECRET, { expiresIn: "1 week" }, (err, token) => {
    if (err)
        reject(err);
    else
        resolve(token);
